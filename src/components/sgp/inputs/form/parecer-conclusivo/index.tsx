import Select from '@/components/lib/inputs/select';
import { ParecerConclusivoDto } from '@/core/dto/ParecerConclusivoDto';
import { ModalidadeEnum } from '@/core/enum/modalidade-enum';
import conselhoClasseService from '@/core/services/conselho-classe-service';
import { Form, FormItemProps, SelectProps } from 'antd';
import { DefaultOptionType } from 'antd/es/select';
import React, { useCallback, useEffect, useState } from 'react';
import { SGP_SELECT_PARECER_CONCLUSIVO } from '~/constantes/ids/select';

type SelectParecerConclusivoProps = {
  anoLetivo: number;
  modalidade: ModalidadeEnum;
} & SelectProps;

const SelectParecerConclusivoAnoLetivoModalidade: React.FC<SelectParecerConclusivoProps> = ({
  anoLetivo,
  modalidade,
  ...rest
}) => {
  const [options, setOptions] = useState<DefaultOptionType[]>([]);
  const [exibirLoader, setExibirLoader] = useState<boolean>(false);

  const obterDados = useCallback(async () => {
    if (!anoLetivo || !modalidade) return;

    setExibirLoader(true);

    const resposta = await conselhoClasseService
      .obterPareceresConclusivosAnoLetivoModalidade(anoLetivo, modalidade)
      .finally(() => setExibirLoader(false));

    if (resposta.sucesso && resposta?.dados?.length) {
      const lista = resposta.dados.map(
        (item: ParecerConclusivoDto): DefaultOptionType => ({
          ...item,
          value: item?.id,
          label: item?.nome,
        }),
      );

      setOptions(lista);
    } else {
      setOptions([]);
    }
  }, [anoLetivo, modalidade]);

  useEffect(() => {
    obterDados();
  }, [obterDados]);

  return (
    <Select
      showSearch
      allowClear
      options={options}
      loading={exibirLoader}
      placeholder="Parecer conclusivo"
      id={SGP_SELECT_PARECER_CONCLUSIVO}
      {...rest}
      value={!exibirLoader ? rest.value : undefined}
    />
  );
};

const SelectParecerConclusivoAnoLetivoModalidadeItem: React.FC<FormItemProps> = ({
  children,
  ...rest
}) => (
  <Form.Item label="Parecer conclusivo" name="parecerConclusivo" {...rest}>
    {children}
  </Form.Item>
);

export {
  SelectParecerConclusivoAnoLetivoModalidade,
  SelectParecerConclusivoAnoLetivoModalidadeItem,
};
