...
Cypress.Commands.add('inserir_turma_registro_acoes_busca', (turma) => {
  ...
  cy.get('body').then(($body) => {
    if (
      $body.find(registroAcoesBuscaSGPlocalizadores.clica_turma_registro_acoes_busca()).length > 0
    ) {
      cy.get(registroAcoesBuscaSGPlocalizadores.clica_turma_registro_acoes_busca(), {
        timeout: 30000,
      })
        .contains(turma.trim())
        .should('be.visible')
...
Cypress.Commands.add('inserir_periodo_registro_acoes_busca', () => {
  cy.get(registroAcoesBuscaSGPlocalizadores.data_inicio_registro_acoes_busca()).click({
    force: true,
  });
  ...
  cy.get(registroAcoesBuscaSGPlocalizadores.ano_registro_acoes_busca()).click();
  cy.get(registroAcoesBuscaSGPlocalizadores.ano_inicio_registro_acoes_busca()).click();
...
Cypress.Commands.add('selecionar_ano_letivo_registro_acoes_busca', () => {
  cy.get(registroAcoesBuscaSGPlocalizadores.ano_letivo_registro_acoes_busca())
    .should('be.visible')
    .click({ force: true });

  cy.get(registroAcoesBuscaSGPlocalizadores.opcao_ano_letivo(), { timeout: 60000 })
    .contains('2025')
    .should('be.visible')
    .click({ force: true })
    .wait(2000);
...
Cypress.Commands.add('dados_carregados_registro_acoes_busca', (cenario) => {
  if (cenario === 'Sem dados encontrados') {
    ...
    cy.get(registroAcoesBuscaSGPlocalizadores.tabela_dados_registro_acoes_busca()).should(
      'not.exist',
    );
  }
});
