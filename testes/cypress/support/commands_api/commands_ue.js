Cypress.Commands.add('buscar_modalidade_ano_letivo_ue', () => {
...
Cypress.Commands.add('nao_autorizado_modalidade_ano_letivo_ue', () => {
    return cy.request({
        method: 'GET',
        url: Cypress.config('baseUrl') + `/api/v1/ues/${Cypress.env('UE_CODIGO')}/modalidades/${Cypress.env('MODALIDADE_CODIGO')}?ano=${Cypress.env('ANO_LETIVO')}`,
        headers: {
...
Cypress.Commands.add('buscar_dados_modalide_ue', () => {
    return cy.gerar_token().then((token) => {        
        return cy.request({
          method: 'GET',
...
Cypress.Commands.add('buscar_dados_modalide_ue_vazio', () => {
    return cy.gerar_token().then((token) => {        
        return cy.request({
          method: 'GET',
...
Cypress.Commands.add('buscar_dados_modalide_ano_vazio', () => {
    return cy.gerar_token().then((token) => {        
        return cy.request({
          method: 'GET',
