...
// Carrega pendências no dashboard de fechamentos
...
Then('carrega pendências no dashboard de fechamentos com status 200', function () {
  cy.get('@response').then((response) => {
  expect(response.status).to.eq(200)
  })
})

// Não retorna pendências sem usuário autenticado
...
Then('não retorna pendências mostrando o status 401', function () {
  cy.get('@response').then((response) => {
